...
-- Regression / smoke tests for the replication_planner extension
-- Run with:  psql -f test/replication_planner_test.sql
-- =============================================================
...
-- Install / reset the planner extension
...
-- Eligible table: has PK, lots of rows, index usage
...
-- Insert enough rows to matter to the planner
...
SELECT
    ...
    (ARRAY['pending','shipped','delivered'])[ceil(random()*3)::int]
FROM generate_series(1, 50000);

ANALYZE test_orders;

-- Eligible table: users
...
SELECT format('user_%s@example.com', g), now() - (random() * INTERVAL '365 days')
FROM generate_series(1, 10000) g;

ANALYZE test_users;

-- Ineligible: no PK, tiny
CREATE TABLE test_tmp_junk (val TEXT);
INSERT INTO test_tmp_junk VALUES ('a'), ('b');

-- -------------------------------------------------------
-- 2. Simulate read workload so pg_stat_user_indexes fills up
-- -------------------------------------------------------
\echo '--- 2. Simulating read workload ---'
...
BEGIN
  FOR i IN 1..200 LOOP
    ...
    PERFORM COUNT(*) FROM test_orders
      WHERE created_at >= now() - INTERVAL '7 days';
    PERFORM id FROM test_users WHERE email = format('user_%s@example.com', i);
  END LOOP;
END;
$$;
...
-- -------------------------------------------------------
\echo '--- 3. Phase 1-A: Index Statistics ---'
...
-- -------------------------------------------------------
\echo '--- 4. Phase 1-B: Table Statistics ---'

SELECT tablename, row_estimate, write_ratio, has_pk,
       eligibility_score, skip_reason
...
-- -------------------------------------------------------
-- 5. Phase 1-C: Workload (pg_stat_statements)
-- -------------------------------------------------------
\echo '--- 5. Phase 1-C: Workload patterns ---'

SELECT heat_tier, calls, call_pct, mean_ms, contains_where, LEFT(query_snippet,80)
FROM   replication_planner.v_hot_queries
LIMIT  5;

-- -------------------------------------------------------
-- 6. Phase 2: Analysis + Scoring
-- -------------------------------------------------------
\echo '--- 6. Phase 2: Heuristic Analysis ---'

SELECT tablename, round(final_score::numeric,4) AS score,
       decision, hot_index_cols, row_filter_hint, skip_reason
FROM   replication_planner.analyze_and_score()
WHERE  tablename IN ('test_orders','test_users','test_tmp_junk')
ORDER  BY final_score DESC;

-- -------------------------------------------------------
-- 7. Phase 3: DDL Generation
-- -------------------------------------------------------
\echo '--- 7. Phase 3: Publication DDL ---'

SELECT publication_name, tablename, decision,
       row_filter,
       ddl_statement
...
-- 8. Full pipeline run
-- -------------------------------------------------------
\echo '--- 8. Full pipeline (rp_run_full_plan) ---'

SELECT replication_planner.run_full_plan();

SELECT tablename, round(final_score::numeric,4) AS score, decision, row_filter_hint
...
SELECT publication_name, tablename, decision, row_filter
FROM   replication_planner.generated_ddl;

-- -------------------------------------------------------
-- 9. Histogram view
-- -------------------------------------------------------
\echo '--- 9. Column histograms ---'

SELECT tablename, column_name, n_distinct, correlation,
       LEFT(top_values,60) AS top_vals
FROM   replication_planner.v_column_histograms
WHERE  tablename IN ('test_orders','test_users')
  AND  column_name IN ('region','status','user_id')
ORDER  BY tablename, column_name;

-- -------------------------------------------------------
-- 10. GUC settings
-- -------------------------------------------------------
\echo '--- 10. GUC settings ---'

SELECT name, setting, short_desc
FROM   replication_planner.v_settings;

-- Adjust a threshold and re-score
SET replication_planner.index_heat_threshold = 0.50;

SELECT name, setting FROM replication_planner.v_settings
